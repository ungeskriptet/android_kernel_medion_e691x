...
{																				
	...
	APS_DBG("LTR-55X ps_thd_val_high=%d, ps_thd_val_low=%d\n.",ps_thd_val_high,ps_thd_val_low);
   	 ...
   	 error = ltr559_i2c_write_reg(LTR559_PS_THRES_UP_0, ps_thd_val_high & 0xff); 
    	if(error<0)
    	{
        	APS_LOG("ltr559_ps_enable setting LTR559_PS_THRES_UP_0 error \n");
       	 return error;
    	} 
   	 error = ltr559_i2c_write_reg(LTR559_PS_THRES_UP_1, (ps_thd_val_high>>8) & 0X07);
    	if(error<0)
    	{
        	APS_LOG("ltr559_ps_enable setting LTR559_PS_THRES_UP_1 error \n");
        	return error;
    	} 
    	error = ltr559_i2c_write_reg(LTR559_PS_THRES_LOW_0, 0x0); 
    	if(error<0)
    	{
        	APS_LOG("ltr559_ps_enable setting LTR559_PS_THRES_LOW_0 error \n");
        	return error;
    	} 
    	error = ltr559_i2c_write_reg(LTR559_PS_THRES_LOW_1, 0x0); 
   	 if(error<0)
    	{
        	APS_LOG("ltr559_ps_enable setting LTR559_PS_THRES_LOW_1 error \n");
        	return error;
    	} 
...
static int ltr559_create_attr(struct device_driver *driver) 
{
	...
	{
               if(0 != (err = driver_create_file(driver, ltr559_attr_list[idx])))
		{            
...
static int ltr559_init_device(void)
{
	...
	als_gainrange = ALS_RANGE_1300;
       APS_FUN();
	error = ltr559_i2c_write_reg(LTR559_PS_LED, 0x11); // 0x1f=0001 1111;0x1b=0001 1011;0x1a=0001 1010;0x12=0001 0010;0x11=0001 0001;
	if(error<0)
    	{
        	APS_LOG("ltr559_ps_enable setting LTR559_PS_LED error...\n");
	    	return error;
	}
	error = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 0xa); 
	if(error<0)
   	{
        	APS_LOG("ltr559_ps_enable setting LTR559_PS_N_PULSES error..\n");
	    	return error;
	} 
	error = ltr559_i2c_write_reg(LTR559_PS_MEAS_RATE, 0x0); 
	if(error<0)
	{
        	APS_LOG("ltr559_ps_enable setting LTR559_PS_MEAS_RATE error\n");
        	return error;
	} 
	error = ltr559_i2c_write_reg(LTR559_ALS_MEAS_RATE, 0x02); 
	if(error<0)
	{
        APS_LOG("ltr559_ps_enable setting LTR559_ALS_MEAS_RATE error.\n");
        return error;
	} 
	...
	{	
		...
		if(res <= 0)
		{
			APS_LOG("LTR559_INTERRUPT error: %x\n",res);
		...
		if(res <= 0)
		{
			APS_LOG("LTR559_INTERRUPT_PERSIST error: %x\n",res);
			return ltr559_ERR_I2C;
		}
...
static int ltr559_devinit(void)
{
	...
	chip_id = ltr559_i2c_read_reg(LTR559_PART_ID);
	if(chip_id == 0x92){
		APS_LOG("LTR-559ALS is detected.\n");
	}else if(chip_id == 0x80){
		APS_LOG("LTR-558ALS is detected.\n");
	}else{
		APS_LOG("Detected LTR-55X error.\n");
	}
...
static int ltr559_get_ps_value(struct ltr559_priv *obj, u16 ps)
{
	...
	APS_DBG("ltr559_get_ps_value: - ps:%d ps_trigger_high:%d ps_trigger_low:%d\n",
...
/*----------------------------------------------------------------------------*/
static int ltr559_als_open_report_data(int open)
{
...
/*----------------------------------------------------------------------------*/
static int ltr559_als_enable_nodata(int enable)
{
	int err=0;
	APS_FUN();	
	if (ltr559_obj == NULL){
		APS_ERR("ltr559_i2c_client is null!!\n");
		return -1;
	}
	
	if (enable > 0) {
		if (0 != (err = ltr559_als_enable(als_gainrange))) {
			APS_ERR("enable als fail: %d\n", err);
			return err;
		}
		set_bit(CMC_BIT_ALS, &ltr559_obj->enable);
	} else {
		if (0 != (err = ltr559_als_disable())) {
			APS_ERR("disable als fail: %d\n", err);
			return err;
		}
...
/*----------------------------------------------------------------------------*/
static int ltr559_als_set_delay(u64 ns)
{
	return 0;
}

/*----------------------------------------------------------------------------*/
static int ltr559_als_get_data(int* value, int* status)
{
	int alsraw=0;
	APS_FUN();
	if (ltr559_obj == NULL) {
		...
		alsraw = ltr559_als_read(als_gainrange);
		*value=  ltr559_get_als_value(ltr559_obj, alsraw);
...
/*----------------------------------------------------------------------------*/
static int ltr559_als_register(void)
{
	int err = 0;
	struct als_control_path als_ctl={0};
	struct als_data_path als_data={0};
	
	als_ctl.open_report_data= ltr559_als_open_report_data;
	als_ctl.enable_nodata = ltr559_als_enable_nodata;
	als_ctl.set_delay  = ltr559_als_set_delay;
	...
	als_data.get_data = ltr559_als_get_data;
...
/*----------------------------------------------------------------------------*/
static int ltr559_ps_open_report_data(int open)
{
	//should queuq work to report event if  is_report_input_direct=true
	return 0;
}
...
/*----------------------------------------------------------------------------*/
static int ltr559_ps_enable_nodata(int enable)
{
	int err=0;
	APS_FUN();	
	if (ltr559_obj == NULL){
	...
	if (enable > 0) {
		if (0 != (err = ltr559_ps_enable(ps_gainrange))) {
			APS_ERR("enable ps fail: %d\n", err);
			return err;
		}
		set_bit(CMC_BIT_PS, &ltr559_obj->enable);
	} else {
		if (0 != (err = ltr559_ps_disable())) {
			APS_ERR("disable ps fail: %d\n", err);
			return err;
		}
...
/*----------------------------------------------------------------------------*/
static int ltr559_ps_set_delay(u64 ns)
{
	return 0;
}

/*----------------------------------------------------------------------------*/
static int ltr559_ps_get_data(int* value, int* status)
{
	int psraw=0;
	...
	if (ltr559_obj == NULL) {
		...
		psraw = ltr559_ps_read();
		*value= ltr559_get_ps_value(ltr559_obj, psraw);
		*status = SENSOR_STATUS_ACCURACY_MEDIUM;
		//APS_ERR("als_get_data *value = %d, *status =%d\n",*value, *status);
	}
...
/*----------------------------------------------------------------------------*/
static int ltr559_ps_register(void)
{
	int err = 0;
	struct ps_control_path ps_ctl={0};
	struct ps_data_path ps_data={0};
	
	ps_ctl.open_report_data= ltr559_ps_open_report_data;
	ps_ctl.enable_nodata = ltr559_ps_enable_nodata;
	ps_ctl.set_delay  = ltr559_ps_set_delay;
	...
	if (err) {
        ...
        if((NULL!=ltr559_obj)){//xmxl
            ps_data.get_data = ltr559_ps_get_data;
...
static int ltr559_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(0 != (err = ltr559_devinit()))
	{
		goto exit_init_failed;
	}
	APS_LOG("ltr559_devinit() ...OK!\n");

	if(0 != (err = misc_register(&ltr559_device)))
	{
		APS_ERR("ltr559_device register failed\n");
		goto exit_misc_device_register_failed;
	}

	if(0 != (err = ltr559_create_attr(&ltr559_init_info.platform_diver_addr->driver)))
	{
		APS_ERR("create attribute err = %d\n", err);
		goto exit_create_attr_failed;
	}

	if ((0 != (err = ltr559_als_register())) || (0 != (err = ltr559_ps_register()))){
...
{
	int err;	
	if(0 != (err = ltr559_delete_attr(&ltr559_i2c_driver.driver)))
	{
		APS_ERR("ltr559_delete_attr fail: %d\n", err);
	} 

	if(0 != (err = misc_deregister(&ltr559_device)))
	{
...
static int  ltr559_local_init(void)
{
	APS_FUN();
	ltr559_power(hw, 1);
	if(0 != (i2c_add_driver(&ltr559_i2c_driver)))
	{
