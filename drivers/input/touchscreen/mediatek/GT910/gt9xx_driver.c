...
u8 is_950 = 0;
u8 esd_resetting = 0;
//xmhzhy delete for permissive error do not filp_open
...
static void force_reset_guitar(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (CHIP_TYPE_GT9F == gtp_chip_type) {
			/* check code ram */
			//xmhzhy fixed for esd do not recovery check, just download anytime. 20160615
			ret = gup_fw_download_proc(NULL, GTP_FL_FW_BURN);//GTP_FL_ESD_RECOVERY
			if (FAIL == ret) {
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	for (i = 0; i < 3; i++) {
		ret = i2c_read_bytes(i2c_client_point,
					     0x8040, esd_buf, 2);
	...
	do {
		...
		if ((finger == 0x00) && (CHIP_TYPE_GT9F == gtp_chip_type)) {
			...
			case GTP_RQST_BAK_REF:
				...
				//xmhzhy delete for permissive error do not filp_open
				//not use to bak ref, not ref bin file
				/*
